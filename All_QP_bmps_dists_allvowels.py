...
train = os.path.join(PARENTDIR,SUB)
subdest = os.path.join(DESTDIR,SUB)
...
for dirs, times, files in os.walk(train):
...
# append some kind of placeholder for when you skip a file
        ts.append(timestamp)
        subjrframelist = []
        utt = os.path.join(train,str(timestamp))
...
## TODO: FIX THIS
       ...
       #         continue
            ...
            imlist = [i for i in os.listdir(utt) if (subjrframelist.search(i) and not regex.search(i))]
            try:
                im = np.array(Image.open(os.path.join(utt,(imlist[0])))) #open one image to get the size
...
#                continue 
            ...
            if rpca is None:
                rpca = np.empty([len(os.listdir(train))]+list(im.shape[0:2])) * np.nan
            difflist = []
            for i in range(imnbr):
                print(i)
                print(imlist[i])
                print(os.path.join(utt,imlist[i]))
  ...
  #              call(["convert", os.path.join(utt,imlist[i]), "-set", "colorspace", "Gray", os.path.join(utt,imlist[i])])
                if i > 0:
                    diffmatrix = np.array(Image.open(os.path.join(utt,imlist[i])))-np.array(Image.open(os.path.join(utt,imlist[i-1])))
	    ...
	    # add frame to array
            ...
            openframe = np.array(Image.open(os.path.join(utt,frame_n)))
		...
		# do the acoustics: find timepoint (time) in file where that frame occurs using the syncfile
            ...
            for v,m in sm.tier('frameidx').search(frame_number, return_match=True):
                ...
                frmtime = v.t1
                print(frmtime)
                frmtimes.append(v.t1)
		# oh, christ. Perform rformant and get F1/F2/F3 at frmtime.
...
outdiffs = os.path.join(DESTDIR,'diffs.txt')
...
dist_headers = ["lan_mean", "basemean", "lan_norm"]
...
# outfile = os.path.join(train, 'dist_calcs.txt')
...
# expl = os.path.join(train, 'explain.txt')
# f = open(expl, 'w')
...
#     pcn = misc.imsave(os.path.join(train,'pc'+str(n+1)+'.png'),pic)
